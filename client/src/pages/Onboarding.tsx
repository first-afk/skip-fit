    ...
    <div className="onboarding-container">
      ...
      <div className="p-6 pt-12 onboarding-wrapper">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Skip Fit</h1>
        </div>
      ...
      <div className="px-6 mb-8 onboarding-wrapper">
        <div className="flex gap-4">
          {[1, 2, 3].map((x) => (
            <div
              key={x}
              className={`h-1.5 flex flex-1 rounded-full transition-all duration-300 ${x <= step ? `bg-emerald-500` : "bg-slate-200 dark:bg-slate-800"}`}
            ></div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="space-y-6">
            <div className="flex items-center gap-4 mb-8">
              <div className="size-12 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 flex items-center justify-center">
          ...
          <div className="space-y-6 onboarding-wrapper">
              ...
              {goalOptions.map((option) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    const range =
                      ageRanges.find((ran) => age <= ran.max) ||
            ...
            <div className="border-t border-slate-200 dark:border-slate-700 my-6 max-w-lg">
              ...
              <div className="space-y-8 max-w-lg">
                ...
                <div className="space-y-6">
                  <Slider label="Daily calorie intake" min={120} max={4000} step={50} value={formData.dailyCalorieIntake} onChange={(v)=> updateField('dailyCalorieIntake', v)} unit="kcal" infoText="Total calories you plan to consume each day"/>
                  <Slider label="Daily calorie burn" min={100} max={2000} step={50} value={formData.dailyCalorieBurn} onChange={(v)=> updateField('dailyCalorieBurn', v)} unit="kcal" infoText="Total calories you plan to burn each day"/>
                </div>
              </div>
            </div>
          </div>
